...
var endpoint = getApi('https://codeandconspire.cdn.prismic.io/api/v2')
...
function documents (state, emitter) {
  ...
  emitter.on('doc:fetch', function (query, opts = {}) {
    if (typeof window === 'undefined') {
      endpoint = getApi('https://codeandconspire.cdn.prismic.io/api/v2')
      if (state.prefetch) {
  ...
  if (state.prefetch) {
    state.prefetch.push(api({type: ['homepage']}))
  }
