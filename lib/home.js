var html = require('choo/html')
var view = require('../components/view')
var poster = require('../components/poster')
var {i18n} = require('../components/base')

var text = i18n()

module.exports = view(home, meta)

function home (state, emit) {
  return html`
    <main class="View-container">
      <div class="View-main">
        <svg class="View-logo" viewBox="0 0 172 69">
          <path fill="currentColor" d="M29.24 4.58H4.8v59.4h43.18V23.22H29.24V4.58zm4.58 3.22v10.85h10.91L33.83 7.8zM.21.02h32.27L52.55 20v48.55H.2V.02zm10.1 35.15l14.98 6.99v3.82l-14.98 7.61v-5.01l9.94-4.51-9.94-3.89v-5.01zm61.57 5.87c-4.42 0-7.68-2.88-7.68-7.31s3.26-7.32 7.68-7.32c1.95 0 3.5.48 5 1.34l-1.86 3.6a5.08 5.08 0 0 0-2.93-.83c-1.8 0-3.37 1.16-3.37 3.2 0 2.06 1.58 3.22 3.37 3.22 1.23 0 2.1-.3 2.93-.83l1.85 3.6a9.55 9.55 0 0 1-4.99 1.33zM84.8 26.41c4.49 0 7.77 2.92 7.77 7.32s-3.28 7.31-7.77 7.31c-4.45 0-7.74-2.91-7.74-7.31 0-4.4 3.29-7.32 7.74-7.32zm0 4.1a3.1 3.1 0 0 0-3.22 3.22 3.1 3.1 0 0 0 3.22 3.2 3.08 3.08 0 0 0 3.23-3.2c0-1.97-1.4-3.21-3.23-3.21zm15.57 10.53c-3.58 0-6.54-2.5-6.54-7.31 0-4.82 2.96-7.32 6.54-7.32 2.24 0 3.65.93 4.51 2.32h.06v-7.85h4.52V40.6h-4.52v-1.88h-.06c-.86 1.4-2.27 2.32-4.5 2.32zm1.26-4.1c1.82 0 3.2-1.25 3.2-3.21a3.06 3.06 0 0 0-3.2-3.21 3.05 3.05 0 0 0-3.17 3.2 3.05 3.05 0 0 0 3.17 3.22zm16.74-10.5c4.15 0 6.84 2.38 6.84 7.35v1h-9.89c.3 1.73 1.67 2.5 3.53 2.5 1.67 0 3.13-.68 4-1.24l1.64 3.39c-1.46.89-3.34 1.6-5.7 1.6-4.64 0-7.71-2.88-7.71-7.25 0-4.64 3.04-7.35 7.29-7.35zm-.15 3.18c-1.53 0-2.66.75-2.87 2.53h5.5c-.06-1.6-1.17-2.53-2.63-2.53zM71.88 62.2c-4.42 0-7.68-2.88-7.68-7.31s3.26-7.32 7.68-7.32c1.95 0 3.5.48 5 1.34l-1.86 3.6a5.08 5.08 0 0 0-2.93-.83c-1.8 0-3.37 1.16-3.37 3.2 0 2.06 1.58 3.22 3.37 3.22 1.23 0 2.1-.3 2.93-.83l1.85 3.6a9.55 9.55 0 0 1-4.99 1.33zM84.8 47.57c4.49 0 7.77 2.92 7.77 7.32s-3.28 7.31-7.77 7.31c-4.45 0-7.74-2.91-7.74-7.31 0-4.4 3.29-7.32 7.74-7.32zm0 4.1a3.1 3.1 0 0 0-3.22 3.22 3.1 3.1 0 0 0 3.22 3.2 3.08 3.08 0 0 0 3.23-3.2c0-1.97-1.4-3.21-3.23-3.21zm18.36-4.1c3.05 0 5.2 1.76 5.2 5.68v8.5h-4.49V54.5c0-2.05-.86-2.97-2.54-2.97-1.52 0-2.66.89-2.66 3.33v6.9h-4.5V48.02h4.41v2.08h.06a5.09 5.09 0 0 1 4.52-2.53zm12.58 14.63c-1.82 0-4.15-.39-6.24-1.43l1.7-3.68c1.5.86 3.2 1.57 4.9 1.57.8 0 1.26-.18 1.26-.62 0-.54-.54-.63-.96-.8l-2.42-.96c-2.27-.86-3.38-2.2-3.38-4.25 0-2.82 1.95-4.46 5.92-4.46 1.55 0 3.23.27 5 .84l-1.26 3.62a12.07 12.07 0 0 0-4.1-1c-.9 0-1.1.23-1.1.59 0 .41.36.6 1.04.86l2.4.9c2.08.76 3.34 1.86 3.34 4.18 0 3.27-2.42 4.64-6.1 4.64zm7.8 5.5V48.02h4.52v1.87h.06c.86-1.4 2.3-2.32 4.5-2.32 3.6 0 6.55 2.5 6.55 7.32 0 4.81-2.96 7.31-6.54 7.31-2.21 0-3.65-.92-4.51-2.32h-.06v7.82h-4.52zm7.83-9.6c1.8 0 3.17-1.25 3.17-3.21a3.05 3.05 0 0 0-3.17-3.21 3.06 3.06 0 0 0-3.2 3.2 3.06 3.06 0 0 0 3.2 3.22zm9.42 3.66V48.02h4.51v13.74h-4.5zm16-14.16v4.5h-.4c-3.01 0-4.24 1.1-4.24 4.6v5.06h-4.5V48.02h4.41v2.8h.06c.81-2.27 2.4-3.22 4.58-3.22h.09zm8.21 0c4.16 0 6.85 2.38 6.85 7.35v1h-9.9c.3 1.73 1.68 2.5 3.53 2.5 1.68 0 3.14-.68 4-1.24l1.65 3.39c-1.46.89-3.35 1.6-5.7 1.6-4.64 0-7.72-2.88-7.72-7.25 0-4.64 3.05-7.35 7.3-7.35zm-.15 3.18c-1.52 0-2.66.75-2.86 2.53h5.5c-.07-1.6-1.17-2.53-2.64-2.53zm-14.7-18.97a41.07 41.07 0 0 1-3.22 4.83l-.04.05 2.97 3.99h-4.97l-.9-1.23-.08.05a9.29 9.29 0 0 1-5.38 1.61c-4.67 0-7.66-2.5-7.66-6.97 0-2.99 1.63-5.3 4.44-6.14l.1-.03-.05-.1a6.08 6.08 0 0 1-.77-2.9c0-2.64 1.83-4.18 5.25-4.18 1.48 0 3.14.3 4.77.81l-1.09 3.64c-1.4-.44-2.32-.65-3.06-.65-.9 0-1.35.35-1.35 1.08 0 .43.2.85.63 1.45l4.53 6.05.08.1.07-.1c.66-.91 1.34-2 2.18-3.44l3.55 2.08zm-11.71 5.15c1.04 0 1.99-.28 2.91-.83l.09-.05-3.72-4.94-.1-.03c-1.48.37-2.43 1.45-2.43 2.88 0 1.84 1.28 2.97 3.25 2.97zM24.64 54.37h17.34v4.56H24.64v-4.56z"/>
        </svg>
        <div class="View-body">
          <h1 class="Display">We are a group of digital experts<span class="u-hiddenVisually"> design and code digital services</span>.</h1>
          <p>We seek elegant technological solutions: code that is <strong>fast</strong> and design that <strong>works</strong>.</p>
          <p>Read <a target="_blank" rel="noopener noreferrer" href="https://www.notion.so/codeandconspire/Manifest-0-4-ab1ec641c36d4c7fae8320bda1bc07dc">our manifest</a> for positive and gratifying digital interactions.</p>
        </div>
        <div class="View-footer">
          <p>
            <a class="View-link" target="_blank" rel="noopener noreferrer" href="https://www.google.se/maps/place/Alma/@59.3215566,18.0169478,13z/data=!4m8!1m2!2m1!1salma!3m4!1s0x465f9d5b00af1afd:0x44480b8d5d69a7c7!8m2!3d59.334149!4d18.077116?dcr=0">Nybrogatan 8, Stockholm</a><br>
            <a class="View-link" href="mailto:hi@codeandconspire.com">hi@codeandconspire.com</a><br>
            <a class="View-link" href="tel:+46735819750">+46 735 819 750</a>
          </p>
          <p class="View-social">
            <a class="View-link" target="_blank" rel="noopener noreferrer" href="https://twitter.com/codeandconspire">Twitter</a><br>
            <a class="View-link" target="_blank" rel="noopener noreferrer" href="https://github.com/codeandconspire">Github</a><br>
            <a class="View-link" target="_blank" rel="noopener noreferrer" href="https://www.facebook.com/codeandconspire">Facebook</a>
          </p>
        </div>
      </div>

      <div class="View-scroll">
        <div class="View-poster View-poster--empty"></div>
        <div class="View-poster">
          ${poster({
    key: 1,
    label: 'United Nations',
    link: {
      text: 'www.globalgoals.org',
      href: 'https://www.globalgoals.org'
    },
    img: {
      src: 'globalgoals.png',
      alt: 'Three stacked screenshots from The Global Goals website'
    },
    title: 'The Global Goals',
    text: 'World leaders agreed to 17 goals for a better world by 2030. We created the website for the agenda, with millions of visitors from all over the world. It\'s a small step in ending poverty, fighting inequality and stopping climate change.'
  })}
        </div>
        <div class="View-poster">
          ${poster({
    key: 2,
    label: 'Maktsalongen',
    link: {
      text: 'litemycket.nu',
      href: 'https://www.litemycket.nu/'
    },
    title: 'Lite mycket nu?',
    text: 'Utbrändhet ökar och det är ett problem för hela samhället. Vi byggde en skola på sms som hjälper dig stå upp mot stress. Redan dag två hade fem tusen personer signat upp.',
    img: {
      src: 'litemycketnu.jpg',
      alt: 'Portrait of Flora Wiström in the Litemycket.nu jacket'
    }
  })}
        </div>
        <div class="View-poster">
          ${poster({
    key: 3,
    label: 'Quantified Planet',
    link: {
      text: 'quantifiedplanet.org',
      href: 'https://www.quantifiedplanet.org/'
    },
    title: 'Real-time data visualized',
    text: 'Quantified Planet needed a logotype, branding and finally a website that would explain a complex product in an inspiring way. It all turned out great.',
    img: {
      src: 'quantifiedplanet.jpg',
      alt: 'Planet Earth, night time'
    }
  })}
        </div>
        <div class="View-poster">
          ${poster({
    key: 4,
    light: true,
    label: 'Amnesty International',
    link: {
      text: 'amnesty.se/unfluencer',
      href: 'https://www.amnesty.se/unfluencer/'
    },
    img: {
      src: 'amnestyunfluencer.png',
      alt: 'Illustrated Twitter bots'
    },
    title: 'Troll factory for the good guys',
    text: 'Amnesty set out to end online herrasment on Twitter, using the same techniques, with tousands of fake account, that Twitter keeps allowing on their platform. An unusial project for sure.'
  })}
        </div>
        <div class="View-poster">
          ${poster({
    key: 5,
    label: 'Planet 15',
    link: {
      text: 'planet15.org',
      href: 'https://planet15.org'
    },
    img: {
      src: 'planet15.jpg',
      alt: 'Rendering of a planet created in the Planet 15 game'
    },
    title: 'Help your city flourish',
    text: 'How do we get a broad group of people to collect data about what grows in their city? We came up with the concept of Planet 15 – a mobile game now under development.'
  })}
        </div>
        <div class="View-poster">
          ${poster({
    key: 7,
    label: 'Gates Foundation',
    link: {
      text: 'Read more about Goalkeepers',
      href: 'https://www.gatesfoundation.org/goalkeepers'
    },
    img: {
      src: 'billgates.jpg',
      alt: 'Bill Gates on the Goalkeepers 2018 state'
    },
    title: 'Progress is possible, but it is not inevitable',
    text: 'We created a highly interactive nomination and voting platform for the Goalkeepers Award, presented during the United Nations General Assembly.'
  })}
        </div>
      </div>
    </main>
  `
}

function meta (state) {
  return {
    'og:image': '/share.png',
    title: text`SITE_TITLE`,
    description: text`SITE_DESCRIPTION`
  }
}
